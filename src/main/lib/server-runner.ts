...
function parseStatsFromLine(serverId: string, line: string): void {
  ...
  // The §-color codes are stripped, but * prefix may remain
...
function startStatsPolling(
  ...
  // Poll every 3 seconds - only when server is ready (world loaded)
  const timer = setInterval(async () => {
    ...
    if (child && child.pid) {
      ...
      const allowProcessMemory =
        !canUseInServerStats(serverId, framework) || Date.now() - lastMemAt > 7000;
