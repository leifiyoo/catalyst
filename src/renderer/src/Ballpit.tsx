...
const DefaultConfig = {
  count: 200,
  colors: [0x6366f1, 0x8b5cf6, 0xd946ef],
...
const Ballpit: React.FC<BallpitProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleResizeNow = () => {
      spheresInstanceRef.current?.resizeNow();
    };
    window.addEventListener('ballpit-resize-now', handleResizeNow as EventListener);

    return () => {
      window.removeEventListener('ballpit-resize-now', handleResizeNow as EventListener);
